...
def custom_forbid(G, G_D, s, n):
    for i in range(len(n.G)): # Setup for G_32 with abstraction
        ...
        s.add( And( Not(G[i] == 136), Not(G[i] == 264), Not(G[i] == 72), Not(G[i] == 1032), Not(G[i] == 520), Not(G[i] == 2056), Not(G[i] == 2176), Not(G[i] == 2304), Not(G[i] == 2112)))

# Generated by impute_forbid.py. Hardcodes some variables based on the abstraction from the previous run.
def custom_hardcode(G, s, n): # Setup for G_32 with abstraction
