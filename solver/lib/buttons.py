...
def custom_forbid(G, G_D, s, n):
    for i in range(len(n.G)): # Setup for G_24 with ab
        ...
        s.add( And( Not(G[i] == 160), Not(G[i] == 288), Not(G[i] == 96), Not(G[i] == 136), Not(G[i] == 264), Not(G[i] == 72), Not(G[i] == 1056), Not(G[i] == 544), Not(G[i] == 2080), Not(G[i] == 1032), Not(G[i] == 520), Not(G[i] == 2056), Not(G[i] == 1088), Not(G[i] == 768), Not(G[i] == 576), Not(G[i] == 2176), Not(G[i] == 2304), Not(G[i] == 2112)))

# Generated by impute_forbid.py. Hardcodes some variables based on the abstraction from the previous run.
def custom_hardcode(G, s, n): # Setup for G_24 with ab
